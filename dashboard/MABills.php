                ...
                <div class="content-wrapper">
                    <div class="row">
                        ...
                        <div class="col-lg-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">Filter Dates</h4><br>
                                    <form id="filterForm" action="MABills.php" method="POST">
                                        <div class="row">
                                    ...
                                    document.getElementById('filterForm').addEventListener('submit', function(e) {
                                        ...
                                        var actionUrl = 'MABills.php?lid=' + encodeURIComponent(lid) +
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <?php
                                        ...
                                        // Execute query for total sales
                                        $sel_today_billNo = "SELECT billNo FROM bill_head WHERE BillingBranch='".$branchCode."' AND DATE(timespam)>='".$fromDate."' AND DATE(timespam)<='".$toDate."' AND cansel=0";
                                        $run_today_billNo = mysqli_query($con, $sel_today_billNo);

                                        while($row_today_billNo = mysqli_fetch_assoc($run_today_billNo)){
                                            // echo $row_today_billNo['billNo'];
                                            $sel_bill_item = "SELECT SUM(TotalPrice) as todayTotalSale FROM bill_data WHERE billNo='".$row_today_billNo['billNo']."' ";
                                            $run_bill_item = mysqli_query($con, $sel_bill_item);
                                            $row_bill_item = mysqli_fetch_assoc($run_bill_item);
                                            $todayTotalSale = $row_bill_item['todayTotalSale'];

                                            // Accumulate total sales
                                            $totalSales += $todayTotalSale;
                                        }
                                    ?>
                                    <h2 class="text-center"><?php echo number_format($totalSales, 2, '.', ','); ?></h2>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <?php
                                        ...
                                        // Execute query for total profit
                                        $sel_today_billNo = "SELECT billNo FROM bill_head WHERE BillingBranch='".$branchCode."' AND DATE(timespam)>='".$fromDate."' AND DATE(timespam)<='".$toDate."' AND cansel=0";
                                        $run_today_billNo = mysqli_query($con, $sel_today_billNo);

                                        while($row_today_billNo = mysqli_fetch_assoc($run_today_billNo)){
                                            $sel_bill_item = "SELECT SUM(TotalPrice) as todayTotalSale, bill_data.ItemCode, items.costPrice, bill_data.QTY, bill_data.Price FROM bill_data,items WHERE bill_data.ItemCode=items.itemCode and bill_data.billNo='".$row_today_billNo['billNo']."' ";
                                            $run_bill_item = mysqli_query($con, $sel_bill_item);
                                            $row_bill_item = mysqli_fetch_assoc($run_bill_item);
                                            $todayTotalSale = $row_bill_item['todayTotalSale'];

                                            // Calculate profit for each item
                                            $bill_cost = $row_bill_item['costPrice'];
                                            $bill_QTY = $row_bill_item['QTY'];
                                            $bill_Price = $row_bill_item['Price'];
                                            $item_profit = $bill_Price - ($bill_QTY * $bill_cost);

                                            // Accumulate total profit
                                            $totalProfit += $item_profit;
                                        }
                                    ?>
                                    <h2 class="text-center"><?php echo number_format($totalProfit, 2, '.', ','); ?></h2>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <?php
                                        ...
                                        $sel_today_bill = "SELECT COUNT(billNo) AS Bill_count FROM bill_head WHERE BillingBranch='".$branchCode."' AND DATE(timespam)>='".$fromDate."' AND DATE(timespam)<='".$toDate."' AND cansel=0";
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <?php
                                        ...
                                        $sel_today_bill = "SELECT SUM(billTotal) AS todayCBillTotal FROM bill_head WHERE BillingBranch='".$branchCode."' AND DATE(timespam)>='".$fromDate."' AND DATE(timespam)<='".$toDate."' AND cansel=2";
                                        $run_today_bill = mysqli_query($con, $sel_today_bill);
                                        $row_today_bill = mysqli_fetch_assoc($run_today_bill);
                                        $todayCBillTotal = $row_today_bill['todayCBillTotal'];
                                        if($todayCBillTotal == ""){
                                            $todayCBillTotal=0;
                                        }
                                    ?>
                                        ...
                                        <div style="overflow-y: auto; max-height: 500px;">
                                            <table class="table table-hover" id="myTable">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $count = 1;
                                                    $sel_query12 = "SELECT * FROM bill_head WHERE BillingBranch='".$branchCode."' and DATE(timespam)>='".$fromDate."' AND DATE(timespam)<='".$toDate."' AND cansel=0";
                                                    $result12 = mysqli_query($con, $sel_query12);
                                                    while ($row = mysqli_fetch_assoc($result12)) {
                                                        $cust_id = $row["customerID"];
                                                        $sel_customer = "SELECT * from customer where id='".$cust_id."'";
                                                ...
                                                ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo $row_customer["name"]; ?>
                                                        <td><?php echo $row_customer["mobile"]; ?>
                                                        </td>
                                                        <td><a class="btn btn-primary btn-sm"
                                                                href="MABillItem.php?lid=<?php echo $lid; ?>&home=<?php echo $home; ?>&billNo=<?php echo $row["billNo"]; ?>&BillingBranch=<?php echo $row["BillingBranch"]; ?>">View
                                                                Bill</a></td>
                                                    </tr>
                                                    <?php $count++; } ?>
                                                </tbody>
