...
$today = date("Y-m-d");
$update_serialST = "UPDATE `retun_items_data` SET `sendDate`='".$today."',`status`='2' WHERE branchCode='".$branchCode."' and itemCode='".$itemCode."' and returnNo='".$returnNo."'";
            ...
            <div class="card-body">
                <div style="border:5px solid; padding:10px;">
                    <div>
                        ...
                        <h4 style="display:flex; align-item:center; justify-content: center; padding:0px; margin:0px"
                            class="card-title">FOULTY RETURN NOTE</h4>
                    </div>
                        ...
                        <table class="table table-hover" id="myTable">
                            <thead>
                                <tr>
                                    ...
                                    <th> SERIAL NO </th>
                                    <th> WARRANTY </th>
                                    <th> DISCRIPTION / FOURLT / GOODS CONDITION </th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php
                                    ...
                                    while($row_return = mysqli_fetch_assoc($run_return)){
                                        ...
                                        $sel_worrenty = "SELECT Warranty from serial_no where serialNo='".$serialNo."' and itemCode='".$itemcode."'";
                                        $run_worrenty = mysqli_query($con,$sel_worrenty);
                                        $row_worrenty = mysqli_fetch_assoc($run_worrenty);
                                        if(mysqli_num_rows($run_worrenty) > 0){
                                            $Warranty = $row_worrenty["Warranty"];
                                            if ($Warranty == '1m') {
                                ...
                                <tr>
                                    <td colspan="5" style="text-align:left; padding:10px;">TECHNICAL OFFICER</td>
                                </tr>
                            </tbody>
                        </table>
