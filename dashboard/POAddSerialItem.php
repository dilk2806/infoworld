                ...
                <div class="content-wrapper">
                    <div class="row">
                        ...
                        <div class="col-lg-12 stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">Add Serial Numbers For <?php echo $itemName; ?></h4>
                                    <!-- <p class="card-description"><code></code> </p>-->
                                    <form
                                        action="POAddSerialItemDB.php?lid=<?php echo $lid; ?>&home=<?php echo $home; ?>&branchCode=<?php echo $branchCode; ?>&PurchesNo=<?php echo $PurchesNo; ?>&itemCode=<?php echo $itemCode; ?>"
                                        method="POST" class="forms-sample" id="serialForm">
                                        <div class="col-md-12 form-group" style="margin-top:0; margin-bottom:0;">
                                        ...
                                        <div class="row">
                                            ...
                                            ?>
                                            <div class="col-md-6 form-group" style="margin-top:0; margin-bottom:0;">
                                                <label for="Serial<?php echo $inputCount; ?>"><b>Serial No.</b></label>
                                                <input type="text" name="Serial<?php echo $inputCount; ?>"
                                                    class="form-control" id="Serial<?php echo $inputCount; ?>"
                                        ...
                                        <div class="col-md-12 form-group"><br>
                                            <button type="button" class="btn btn-primary" onclick="validateForm()">Check
                                                Duplicate Serials</button>
                                            <button type="submit" id="submitBTN" class="btn btn-success"><b>Add Serial &
                                                    Warranty</b></button>
                                        </div>
                                    </form>
                                    ...
                                    function validateForm() {
                                        ...
                                        <?php
                                            // PHP code to fetch serial numbers from database
                                            $sel_serials = "SELECT serialNo from serial_no where itemCode='".$itemCode."'";
                                            $run_serials = mysqli_query($con,$sel_serials);
                                            while($row_serials = mysqli_fetch_assoc($run_serials)) {
                                        ?>
                                        purchedSerials.push(
                                            '<?php echo addslashes($row_serials['serialNo']); ?>'
                                            ); // Ensure proper escaping
                                        <?php } ?>

                                        console.log(purchedSerials); // Output to console for debugging

                                        // Loop through each input field named 'SerialX'
                                        var inputs = document.querySelectorAll('input[name^="Serial"]');
                                        console.log("inputs: ", inputs);

                                        inputs.forEach(function(input) {
                                            var serialNumber = input.value.trim();

                                            if (serialNumber !== '') {
                                                if (purchedSerials.includes(serialNumber)) {
                                                    alreadyadded.push(serialNumber);
                                                } else {
                                        ...
                                        if (duplicates.length > 0) {
                                            alert(duplicates.length + ' Duplicate serial number(s) found now: ' +
                                                duplicates.join(', '));
                                        } else {
                                            alert('No duplicate serial number(s) found now.');
                                        }
                                        if (alreadyadded.length > 0) {
                                            alert(alreadyadded.length + ' Already added serial number(s) found now: ' +
                                                alreadyadded.join(', '));
                                        }
                                    }
                                    </script>
