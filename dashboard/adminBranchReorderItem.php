...
$lid = $_REQUEST['lid'];
$home = $_REQUEST['home'];
$branchCodeReorder = $_REQUEST['branchCode'];
                ...
                <div class="content-wrapper">
                    <div class="row">
                        ...
                        <div class="col-lg-12 stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">Re-Ordering Item List</h4>
                                        ...
                                        <div style="overflow-y: auto; max-height: 500px;">
                                            <table class="table table-hover" id="myTable">
                                                ...
                                                <tbody>
                                                    <?php
                                                    $count = 1;
                                                    $sel_query12 = "SELECT stock.qty,items.reorderLevel,items.itemName,items.itemCode,brand_names.brandName,categories.catName FROM items,categories,brand_names,stock
                                                    where items.itemCode=stock.itemCode and items.catCode=categories.catCode and items.brandCode=brand_names.brandCode and stock.branchCode='".$branchCodeReorder."' ORDER BY itemCode ASC";
                                                    $result12 = mysqli_query($con, $sel_query12);
                                                    while ($row = mysqli_fetch_assoc($result12)) {
                                                        if($row['qty'] <= $row['reorderLevel']){
                                                            $supplierList = [];
                                                            $itemCode = $row["itemCode"];
                                                            $sel_supplier = "SELECT suplier.SuplierName 
                                                                            FROM suplier 
                                                                            JOIN supplier_item ON suplier.SuplierCode = supplier_item.supplier_code 
                                                                            WHERE supplier_item.item_code = $itemCode";
                                                            $run_supplier = mysqli_query($con, $sel_supplier);

                                                            while ($row_supplier = mysqli_fetch_assoc($run_supplier)) {
                                                                $supplierList[] = $row_supplier['SuplierName'];
                                                            }
                                                            $supplierNames = implode("<br>", $supplierList);
